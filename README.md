...
- **uTLS** с выбираемым пулом отпечатков (chrome / firefox / safari / edge / ios / yandex),
  применяется и к импортным конфигам; **фрагментация TLS ClientHello** для разрезания SNI.
- **Нативная ECH-маскировка (как у Chrome)** — при включённом ECH приложение само находит
  опубликованный в DNS ECH-ключ узла (через зашифрованный DoH) и **шифрует реальное TLS-имя сервера**
  внутри хендшейка: в эфире виден только прикрывающий `public_name` (для узла за Cloudflare — имя
  Cloudflare). Без своего бинаря — то же преимущество, что проприетарные «маскировочные» VPN получают
  отдельной программой, здесь на штатном ядре. Лучше всего для узлов за Cloudflare / своего ECH-эндпоинта;
  на голом узле без ECH — no-op (поиск best-effort, безопасно отваливается по таймауту). Reality не
  трогаем (он прячет своё имя иначе).
...
- **Нативная разблокировка Telegram без сервера (движок `tgcore`)** — забандленный локальный MTProxy
  мостит Telegram к его *незадушенному веб-шлюзу* по WebSocket, замаскированному под TLS *твоего*
  браузера (отпечаток снимается один раз, автоматически). Telegram — **и сообщения, и медиа** — идёт
  по чистому пути, похожему на обычный браузерный HTTPS, **без зарубежного сервера**. Включи тумблер
  (Настройки → Продвинутые → «Telegram без сервера») и нажми «Открыть в Telegram» — прокси добавится в
  один тап. База без админа; опциональные **звонки** — пакетный STUN-десинк (нужен админ). Честно:
  работает *из-под* ТСПУ (незадушенный шлюз чист только под фильтром), это про надёжность + медиа, а не
  про скорость; полный региональный IP-shutdown всё равно требует сервера.
- **Через свой сервер (когда даже шлюз закрыт по IP)** — при подключении приложение **автоматически**
  пинит опубликованные CIDR DC/релеев Telegram + домены на зарубежный выход. **Сообщения** идут в
  любом режиме; **звонки** по UDP требуют **TUN**. Совет: укажи у Telegram встроенный **SOCKS5** на
  `127.0.0.1:2080` (Настройки → Продвинутые → Использовать прокси → SOCKS5) — он слушает всегда (и
  proxy-, и TUN-режим), так что Telegram сам пойдёт через туннель **и сообщениями, и звонками**, не
  завися от режима.
...
port-hopping для Hysteria2, нативная разблокировка Telegram без сервера (движок `tgcore`) + пин на
сервер, **обучаемая память сети** (каскад
