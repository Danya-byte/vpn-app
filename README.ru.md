...
подключением — протокол, сервер, SNI, бейдж *insecure* (без проверки серта), громкое предупреждение
...
- **Нативная разблокировка Telegram** — пинит *опубликованные* диапазоны IP дата-центров/релеев
  Telegram (и его UDP-звонки в режиме TUN) на прокси-выход, так что и сообщения, **и** звонки идут
  через туннель, даже когда сами IP Telegram заблокированы — без возни с MTProto-прокси.
...
port-hopping для Hysteria2, нативная разблокировка Telegram, FakeIP DNS, синхронизация профилей по
...
Осталось: проверка kill-switch на утечки на железе (до включения по умолчанию); пред-коннект замер
задержки по всему профилю; пиннинг серта hy2; UI для ECS/ECH; шер «по близости» (как AirDrop); сборки
под macOS и Android.
