...
bool Win32Window::Create(const std::wstring& title,
  ...
  HWND window = CreateWindow(
      // No maximize box: this is a compact, phone-shaped window.
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_GETMINMAXINFO: {
      // Enforce a min size, and cap the max so the window can't be blown up to
      // fullscreen (it stays a compact, phone-shaped window).
      ...
      info->ptMaxTrackSize.x = static_cast<LONG>(620 * scale);
      info->ptMaxTrackSize.y = static_cast<LONG>(1100 * scale);
      return 0;
    }
